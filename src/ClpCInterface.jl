...
mutable struct ClpModel
    p::Ptr{Nothing}
    function ClpModel()
        p = @clp_ccall newModel Ptr{Nothing} ()
        prob = new(p)
...
function delete_model(model::ClpModel)
    ...
    @clp_ccall deleteModel Nothing (Ptr{Nothing},) model.p
    model.p = C_NULL
    return
end
...
mutable struct ClpSolve
    p::Ptr{Nothing}
    function ClpSolve()
        p = @clpsolve_ccall new Ptr{Nothing} ()
        prob = new(p)
...
function delete_solve(solve::ClpSolve)
    ...
    @clpsolve_ccall delete Nothing (Ptr{Nothing},) solve.p
    solve.p = C_NULL
    return
end
...
function _jl__check_row_is_valid(model::ClpModel, row::Integer)
    num_rows = @clp_ccall getNumRows Int32 (Ptr{Nothing},) model.p
    if !(1 <= row <= num_rows)
...
function _jl__check_col_is_valid(model::ClpModel, col::Integer)
    num_cols = @clp_ccall getNumCols Int32 (Ptr{Nothing},) model.p
    if !(1 <= col <= num_cols)
...
const VecOrNothing = Union{Vector,Nothing}
function vec_or_null(::Type{T}, a::VecOrNothing, len::Integer) where T
    if isequal(a, nothing) || isa(a, Array{Nothing}) # on 0.3, [] is Array{Nothing}
        return C_NULL
...
function load_problem(model::ClpModel,  num_cols::Integer, num_rows::Integer,
    ...
    @clp_ccall loadProblem Nothing (Ptr{Nothing},Int32,Int32,Ptr{CoinBigIndex},Ptr{Int32},
...
function load_quadratic_objective(model::ClpModel,
    ...
    _jl__check_model(model)
    @clp_ccall loadQuadraticObjective Nothing (Ptr{Nothing},Int32,Ptr{CoinBigIndex},Ptr{Int32},Ptr{Float64}) model.p num_cols start column element
end
...
function read_mps(model::ClpModel, mpsfile::AbstractString, keep_names::Bool, ignore_errors::Bool)
    ...
    status = @clp_ccall readMps Int32 (Ptr{Nothing}, Cstring, Int32, Int32) model.p mpsfile keep_names ignore_errors
    if status != 0
...
function copy_in_integer_information(model::ClpModel, information::Vector{UInt8})
    ...
    @clp_ccall copyInIntegerInformation Nothing (Ptr{Nothing},Ptr{UInt8}) model.p information
end

# Drop integer information.
function delete_integer_information(model::ClpModel)
    _jl__check_model(model)
    @clp_ccall deleteIntegerInformation Nothing (Ptr{Nothing},) model.p
end

# Resize rim part of model.
function resize(model::ClpModel, new_num_rows::Integer, new_num_cols::Integer)
    _jl__check_model(model)
    @clp_ccall resize Nothing (Ptr{Nothing},Int32,Int32) model.p new_num_rows new_num_cols
end

# Delete rows.
function delete_rows(model::ClpModel, which::Vector{Int32})
    _jl__check_model(model)
    @clp_ccall deleteRows Nothing (Ptr{Nothing},Int32,Ptr{Int32}) model.p length(which) which
end
...
function add_rows(model::ClpModel, number::Integer, row_lower::Vector{Float64},
    ...
    @clp_ccall addRows Nothing (Ptr{Nothing}, Int32, Ptr{Float64}, Ptr{Float64}, Ptr{Int32}, Ptr{Int32}, Ptr{Float64}) model.p number row_lower row_upper row_starts columns elements
end

# Delete columns.
function delete_columns(model::ClpModel, which::Vector{Int32})
    _jl__check_model(model)
    @clp_ccall deleteColumns Nothing (Ptr{Nothing},Int32,Ptr{Int32}) model.p length(which) which
end
...
function add_columns(model::ClpModel, number::Integer, column_lower::Vector{Float64},
    ...
    @clp_ccall addColumns Nothing (Ptr{Nothing}, Int32, Ptr{Float64}, Ptr{Float64}, Ptr{Float64}, Ptr{Int32}, Ptr{Int32}, Ptr{Float64}) model.p number column_lower column_upper objective column_starts rows elements
...
function chg_row_lower(model::ClpModel, row_lower::Vector{Float64})
    ...
    @clp_ccall chgRowLower Nothing (Ptr{Nothing}, Ptr{Float64}) model.p row_lower
end
...
function chg_row_upper(model::ClpModel, row_upper::Vector{Float64})
    ...
    @clp_ccall chgRowUpper Nothing (Ptr{Nothing}, Ptr{Float64}) model.p row_upper
end
...
function chg_column_lower(model::ClpModel, column_lower::Vector{Float64})
    ...
    @clp_ccall chgColumnLower Nothing (Ptr{Nothing}, Ptr{Float64}) model.p column_lower
end
...
function chg_column_upper(model::ClpModel, column_upper::Vector{Float64})
    ...
    @clp_ccall chgColumnUpper Nothing (Ptr{Nothing}, Ptr{Float64}) model.p column_upper
end
...
function chg_obj_coefficients(model::ClpModel, obj_in::Vector{Float64})
    ...
    @clp_ccall chgObjCoefficients Nothing (Ptr{Nothing},Ptr{Float64}) model.p obj_in
end
...
function drop_names(model::ClpModel)
    ...
    @clp_ccall dropNames Nothing (Ptr{Nothing},) model.p
end
...
function get_num_rows(model::ClpModel)
    _jl__check_model(model)
    @clp_ccall getNumRows Int32 (Ptr{Nothing},) model.p
end
...
function get_num_cols(model::ClpModel)
    _jl__check_model(model)
    @clp_ccall getNumCols Int32 (Ptr{Nothing},) model.p
end
...
function primal_tolerance(model::ClpModel)
    _jl__check_model(model)
    @clp_ccall primalTolerance Float64 (Ptr{Nothing},) model.p
end

# Set primal tolerance to use.
function set_primal_tolerance(model::ClpModel, value::Float64)
    _jl__check_model(model)
    @clp_ccall setPrimalTolerance Nothing (Ptr{Nothing}, Float64) model.p value
    return
end
...
function dual_tolerance(model::ClpModel)
    _jl__check_model(model)
    @clp_ccall dualTolerance Float64 (Ptr{Nothing},) model.p
end

# Set dual tolerance to use.
function set_dual_tolerance(model::ClpModel, value::Float64)
    _jl__check_model(model)
    @clp_ccall setDualTolerance Nothing (Ptr{Nothing}, Float64) model.p value
    return
end
...
function dual_objective_limit(model::ClpModel)
    _jl__check_model(model)
    @clp_ccall dualObjectiveLimit Float64 (Ptr{Nothing},) model.p
end

# Set dual objective limit.
function set_dual_objective_limit(model::ClpModel, value::Float64)
    @clp_ccall setDualObjectiveLimit Nothing (Ptr{Nothing}, Float64) model.p value
    return
end
...
function set_objective_offset(model::ClpModel, value::Float64)
    @clp_ccall setObjectiveOffset Nothing (Ptr{Nothing}, Float64) model.p value
    return
end
...
function problem_name(model::ClpModel)
    ...
    @clp_ccall problemName Nothing (Ptr{Nothing}, Int32, Ptr{UInt8}) model.p 1000 problem_name
...
function set_problem_name(model::ClpModel, name::String)
    ...
    @clp_ccall setProblemName Nothing (Ptr{Nothing}, Int32, Ptr{UInt8}) model.p (length(name)+1) bytestring(name)
end
=#
...
function number_iterations(model::ClpModel)
    _jl__check_model(model)
    @clp_ccall numberIterations Int32 (Ptr{Nothing},) model.p
end
...
function set_number_iterations(model::ClpModel, iters::Integer)
    _jl__check_model(model)
    @clp_ccall setNumberIterations Nothing (Ptr{Nothing}, Int32) model.p iters
    return
end
...
function maximum_iterations(model::ClpModel)
    _jl__check_model(model)
    @clp_ccall maximumIterations Int32 (Ptr{Nothing},) model.p
end

# Set maximum number of iterations
function set_maximum_iterations(model::ClpModel, max_iters::Integer)
    _jl__check_model(model)
    @clp_ccall setMaximumIterations Nothing (Ptr{Nothing}, Int32) model.p max_iters
    return
end
...
function maximum_seconds(model::ClpModel)
    _jl__check_model(model)
    @clp_ccall maximumSeconds Float64 (Ptr{Nothing},) model.p
end

# Set maximum time in seconds (from when set is called)
function set_maximum_seconds(model::ClpModel, max_secs::Real)
    _jl__check_model(model)
    @clp_ccall setMaximumSeconds Nothing (Ptr{Nothing}, Float64) model.p max_secs
    return
end
...
function hit_maximum_iterations(model::ClpModel)
    _jl__check_model(model)
    ret = @clp_ccall hitMaximumIterations Int32 (Ptr{Nothing},) model.p
    return ret != 0
end
...
function status(model::ClpModel)
    _jl__check_model(model)
    @clp_ccall status Int32 (Ptr{Nothing},) model.p
end

# Set the status of the problem.
function set_problem_status(model::ClpModel, status::Integer)
    _jl__check_model(model)
    @clp_ccall setProblemStatus Nothing (Ptr{Nothing}, Int32) model.p status
    return
end
...
function secondary_status(model::ClpModel)
    _jl__check_model(model)
    @clp_ccall secondaryStatus Int32 (Ptr{Nothing},) model.p
end

# Set the secondary status of the problem.
function set_secondary_status(model::ClpModel, status::Integer)
    _jl__check_model(model)
    @clp_ccall setSecondaryStatus Nothing (Ptr{Nothing}, Int32) model.p status
    return
end
...
function optimization_direction(model::ClpModel)
    _jl__check_model(model)
    @clp_ccall optimizationDirection Float64 (Ptr{Nothing},) model.p
end
...
function set_optimization_direction(model::ClpModel, value::Real)
    _jl__check_model(model)
    @clp_ccall setOptimizationDirection Nothing (Ptr{Nothing}, Float64) model.p value
    return
end
    ...
    if VERSION >= v"0.7-"
        return quote
            function $(esc(fname))(model::ClpModel)
                _jl__check_model(model)
                row_property_p = @clp_ccall $clpname Ptr{Float64} (Ptr{Nothing},) model.p
        ...
        return quote
            function $(esc(fname))(model::ClpModel)
                _jl__check_model(model)
                row_property_p = @clp_ccall $(esc(clpname)) Ptr{Float64} (Ptr{Nothing},) model.p
                num_rows = convert(Int,get_num_rows(model))
                return copy(unsafe_wrap(Array,row_property_p,(num_rows,)))
            end
        end
...
macro def_get_col_property(fname,clpname)
    if VERSION >= v"0.7-"
        return quote
            function $(esc(fname))(model::ClpModel)
                _jl__check_model(model)
                col_property_p = @clp_ccall $clpname Ptr{Float64} (Ptr{Nothing},) model.p
        ...
        return quote
            function $(esc(fname))(model::ClpModel)
                _jl__check_model(model)
                col_property_p = @clp_ccall $(esc(clpname)) Ptr{Float64} (Ptr{Nothing},) model.p
                num_cols = convert(Int,get_num_cols(model))
                return copy(unsafe_wrap(Array,col_property_p,(num_cols,)))
            end
        end
...
function set_col_solution(model::ClpModel, input::Vector{Float64})
    ...
    @clp_ccall setColSolution Nothing (Ptr{Nothing},Ptr{Float64}) model.p input
end
...
function get_num_elements(model::ClpModel)
    _jl__check_model(model)
    @clp_ccall getNumElements Int32 (Ptr{Nothing},) model.p
end

# Get column starts in matrix.
function get_vector_starts(model::ClpModel)
    _jl__check_model(model)
    vec_starts_p = @clp_ccall getVectorStarts Ptr{CoinBigIndex} (Ptr{Nothing},) model.p
...
function get_indices(model::ClpModel)
    ...
    row_indices_p = @clp_ccall getIndices Ptr{Int32} (Ptr{Nothing},) model.p
...
function get_elements(model::ClpModel)
    _jl__check_model(model)
    elements_p = @clp_ccall getElements Ptr{Float64} (Ptr{Nothing},) model.p
    num_elts = Int(get_num_elements(model))
...
function get_obj_value(model::ClpModel)
    _jl__check_model(model)
    @clp_ccall getObjValue Float64 (Ptr{Nothing},) model.p
end
...
function infeasibility_ray(model::ClpModel)
    _jl__check_model(model)
    infeas_ray_p = @clp_ccall infeasibilityRay Ptr{Float64} (Ptr{Nothing},) model.p
    num_rows = convert(Int,get_num_rows(model))
    local infeas_ray::Vector{Float64}
    if infeas_ray_p != C_NULL
        infeas_ray = copy(unsafe_wrap(Array,infeas_ray_p,(num_rows,)))
        ccall(:free,Nothing,(Ptr{Nothing},),infeas_ray_p)
    else
...
function unbounded_ray(model::ClpModel)
    _jl__check_model(model)
    unbd_ray_p = @clp_ccall unboundedRay Ptr{Float64} (Ptr{Nothing},) model.p
    num_cols = convert(Int,get_num_cols(model))
    local unbd_ray::Vector{Float64}
    if unbd_ray_p != C_NULL
        unbd_ray = copy(unsafe_wrap(Array,unbd_ray_p,(num_cols,)))
        ccall(:free,Nothing,(Ptr{Nothing},),unbd_ray_p)
    else
...
function statusExists(model::ClpModel)
    _jl__check_model(model)
    ret = @clp_ccall statusExists Int32 (Ptr{Nothing},) model.p
    return ret != 0
end
...
function get_column_status(model::ClpModel, sequence::Integer)
    ...
    return @clp_ccall getColumnStatus Int32 (Ptr{Nothing},Int32) model.p (sequence-1)
end
...
function get_row_status(model::ClpModel, sequence::Integer)
    ...
    return @clp_ccall getRowStatus Int32 (Ptr{Nothing},Int32) model.p (sequence-1)
end
...
function set_column_status(model::ClpModel, sequence::Integer, value::Integer)
    _jl__check_model(model)
    _jl__check_col_is_valid(model, sequence)
    @clp_ccall setColumnStatus Nothing (Ptr{Nothing},Int32,Int32) model.p (sequence-1) value
end
...
function set_row_status(model::ClpModel, sequence::Integer, value::Integer)
    _jl__check_model(model)
    _jl__check_row_is_valid(model, sequence)
    @clp_ccall setRowStatus Nothing (Ptr{Nothing},Int32,Int32) model.p (sequence-1) value
end
...
#function set_user_pointer(model::ClpModel, pointer::Ptr{Nothing})

# Pass in c-pointer to callback function. (from cfunction())
# See Coin_C_defines.h for signature
# TODO: test this
function register_call_back(model::ClpModel, callback::Ptr{Nothing})
    _jl__check_model(model)
    @clp_ccall registerCallBack Nothing (Ptr{Nothing},Ptr{Nothing}) model.p callback
end

# Unset callback function.
function clear_call_back(model::ClpModel)
    _jl__check_model(model)
    @clp_ccall clearCallBack Nothing (Ptr{Nothing},) model.p
    return
end
...
function log_level(model::ClpModel)
    _jl__check_model(model)
    @clp_ccall logLevel Int32 (Ptr{Nothing},) model.p
end

# Set amount of print out.
function set_log_level(model::ClpModel, value::Integer)
    _jl__check_model(model)
    @clp_ccall setLogLevel Nothing (Ptr{Nothing}, Int32) model.p value
    return
end
...
function length_names(model::ClpModel)
    _jl__check_model(model)
    return @clp_ccall lengthNames Int32 (Ptr{Nothing},) model.p
end

# Return an array with a row name.
# XXX deviation from Clp API syntax, which fills a user-provided
# array of chars:
#   Nothing Clp_rowName(Clp_Simplex * model, int iRow, char * name);
function row_name(model::ClpModel, row::Integer)
    _jl__check_model(model)
    _jl__check_row_is_valid(model, row)
    size = @clp_ccall lengthNames Int32 (Ptr{Nothing},) model.p
    row_name = Array{UInt8}(size+1)
    @clp_ccall rowName Nothing (Ptr{Nothing}, Int32, Ptr{UInt8}) model.p (row-1) row_name
...
# XXX deviation from Clp API syntax, which fills a user-provided
# array of chars:
#   Nothing Clp_columnName(Clp_Simplex * model, int iColumn, char * name);
function column_name(model::ClpModel, col::Integer)
    _jl__check_model(model)
    _jl__check_col_is_valid(model, col)
    size = @clp_ccall lengthNames Int32 (Ptr{Nothing},) model.p
    col_name = Array{UInt8}(size+1)
    @clp_ccall columnName Nothing (Ptr{Nothing}, Int32, Ptr{UInt8}) model.p (col-1) col_name
...
function initial_solve(model::ClpModel)
    _jl__check_model(model)
    @clp_ccall initialSolve Int32 (Ptr{Nothing},) model.p
end

function initial_solve_with_options(model::ClpModel,solve::ClpSolve)
    _jl__check_model(model)
    _jl__check_solve(solve)
    @clp_ccall initialSolveWithOptions Int32 (Ptr{Nothing},Ptr{Nothing}) model.p solve.p
end
...
function initial_dual_solve(model::ClpModel)
    _jl__check_model(model)
    @clp_ccall initialDualSolve Int32 (Ptr{Nothing},) model.p
end

# Primal initial solve.
function initial_primal_solve(model::ClpModel)
    _jl__check_model(model)
    @clp_ccall initialPrimalSolve Int32 (Ptr{Nothing},) model.p
end

# Barrier initial solve.
function initial_barrier_solve(model::ClpModel)
    _jl__check_model(model)
    @clp_ccall initialBarrierSolve Int32 (Ptr{Nothing},) model.p
end

# Barrier initial solve, no crossover.
function initial_barrier_no_cross_solve(model::ClpModel)
    _jl__check_model(model)
    @clp_ccall initialBarrierNoCrossSolve Int32 (Ptr{Nothing},) model.p
end

# Dual algorithm.
function dual(model::ClpModel, ifValuesPass::Integer)
    _jl__check_model(model)
    @clp_ccall dual Int32 (Ptr{Nothing},Int32) model.p ifValuesPass
end

# Primal algorithm.
function primal(model::ClpModel, ifValuesPass::Integer)
    _jl__check_model(model)
    @clp_ccall primal Int32 (Ptr{Nothing},Int32) model.p ifValuesPass
end

# Solve the problem with the idiot code.
function idiot(model::ClpModel, tryhard::Integer)
    _jl__check_model(model)
    @clp_ccall idiot Int32 (Ptr{Nothing},Int32) model.p tryhard
end
...
function scaling(model::ClpModel, mode::Integer)
    ...
    @clp_ccall scaling Nothing (Ptr{Nothing}, Int32) model.p mode
    return
end
...
function scaling_flag(model::ClpModel)
    _jl__check_model(model)
    return @clp_ccall scalingFlag Int32 (Ptr{Nothing},) model.p
end
...
function crash(model::ClpModel, gap::Float64, pivot::Int32)
    ...
    return @clp_ccall crash Int32 (Ptr{Nothing}, Float64, Int32) model.p gap pivot
end

# Query whether the problem is primal feasible.
function primal_feasible(model::ClpModel)
    _jl__check_model(model)
    ret = @clp_ccall primalFeasible Int32 (Ptr{Nothing},) model.p
    return ret != 0
end
...
function dual_feasible(model::ClpModel)
    _jl__check_model(model)
    ret = @clp_ccall dualFeasible Int32 (Ptr{Nothing},) model.p
    return ret != 0
end
...
function dual_bound(model::ClpModel)
    _jl__check_model(model)
    @clp_ccall dualBound Float64 (Ptr{Nothing},) model.p
end

# Set dual bound.
function set_dual_bound(model::ClpModel, value::Real)
    _jl__check_model(model)
    @clp_ccall setDualBound Nothing (Ptr{Nothing}, Float64) model.p value
    return
end
...
function infeasibility_cost(model::ClpModel)
    _jl__check_model(model)
    @clp_ccall infeasibilityCost Float64 (Ptr{Nothing},) model.p
end

# Set infeasibility cost.
function set_infeasibility_cost(model::ClpModel, value::Real)
    _jl__check_model(model)
    @clp_ccall setInfeasibilityCost Nothing (Ptr{Nothing}, Float64) model.p value
    return
end
...
function perturbation(model::ClpModel)
    _jl__check_model(model)
    @clp_ccall perturbation Int32 (Ptr{Nothing},) model.p
end
...
function set_perturbation(model::ClpModel, value::Integer)
    ...
    @clp_ccall setPerturbation Nothing (Ptr{Nothing}, Int32) model.p value
    return
end
...
function algorithm(model::ClpModel)
    _jl__check_model(model)
    @clp_ccall algorithm Int32 (Ptr{Nothing},) model.p
end
...
function set_algorithm(model::ClpModel, value::Integer)
    ...
    @clp_ccall setAlgorithm Nothing (Ptr{Nothing}, Int32) model.p value
    return
end
...
function save_model(model::ClpModel, file_name::String)
    ...
    @clp_ccall saveModel Int32 (Ptr{Nothing}, Cstring) model.p file_name
end
...
function restore_model(model::ClpModel, file_name::String)
    ...
    @clp_ccall restoreModel Int32 (Ptr{Nothing}, Cstring) model.p file_name
end
...
function check_solution(model::ClpModel);
    _jl__check_model(model)
    @clp_ccall checkSolution Nothing (Ptr{Nothing},) model.p
    return
end
...
function is_abandoned(model::ClpModel)
    _jl__check_model(model)
    ret = @clp_ccall isAbandoned Int32 (Ptr{Nothing},) model.p
    return ret != 0
end
...
function is_proven_optimal(model::ClpModel)
    _jl__check_model(model)
    ret = @clp_ccall isProvenOptimal Int32 (Ptr{Nothing},) model.p
    return ret != 0
end
...
function is_proven_primal_infeasible(model::ClpModel)
    _jl__check_model(model)
    ret = @clp_ccall isProvenPrimalInfeasible Int32 (Ptr{Nothing},) model.p
    return ret != 0
end
...
function is_proven_dual_infeasible(model::ClpModel)
    _jl__check_model(model)
    ret = @clp_ccall isProvenDualInfeasible Int32 (Ptr{Nothing},) model.p
    return ret != 0
end
...
function is_primal_objective_limit_reached(model::ClpModel)
    _jl__check_model(model)
    ret = @clp_ccall isPrimalObjectiveLimitReached Int32 (Ptr{Nothing},) model.p
    return ret != 0
end
...
function is_dual_objective_limit_reached(model::ClpModel)
    _jl__check_model(model)
    ret = @clp_ccall isDualObjectiveLimitReached Int32 (Ptr{Nothing},) model.p
    return ret != 0
end
...
function is_iteration_limit_reached(model::ClpModel)
    _jl__check_model(model)
    ret = @clp_ccall isIterationLimitReached Int32 (Ptr{Nothing},) model.p
    return ret != 0
end
...
function get_obj_sense(model::ClpModel)
    _jl__check_model(model)
    @clp_ccall getObjSense Float64 (Ptr{Nothing},) model.p
end
...
function set_obj_sense(model::ClpModel, objsen::Real)
    ...
    @clp_ccall setObjSense Nothing (Ptr{Nothing}, Float64) model.p objsen
    return
end
...
function print_model(model::ClpModel, prefix::String)
    ...
    @clp_ccall printModel Nothing (Ptr{Nothing}, Cstring) model.p prefix
end

function get_small_element_value(model::ClpModel)
    _jl__check_model(model)

    @clp_ccall getSmallElementValue Float64 (Ptr{Nothing},) model.p
end

function set_small_element_value(model::ClpModel,value::Float64)
    _jl__check_model(model)

    @clp_ccall setSmallElementValue Nothing (Ptr{Nothing},Float64) model.p value
end
...
function set_special_option(solve::ClpSolve, which::Integer, value::Integer, extraInfo::Integer)
    ...
    @clpsolve_ccall setSpecialOption Nothing (Ptr{Nothing},Int32,Int32,Int32) solve.p which value extraInfo
end

function get_special_option(solve::ClpSolve, which::Integer)
    _jl__check_solve(solve)

    @clpsolve_ccall getSpecialOption Int32 (Ptr{Nothing},Int32) solve.p which
end
...
macro def_get_int_property(fname,clpname)
    quote
        function $(esc(fname))(solve::ClpSolve)
            _jl__check_solve(solve)
            @clpsolve_ccall $clpname Int32 (Ptr{Nothing},) solve.p
        end
    end
end
...
macro def_set_int_property(fname,clpname)
    quote
        function $(esc(fname))(solve::ClpSolve, val::Integer)
            _jl__check_solve(solve)
            @clpsolve_ccall $clpname Nothing (Ptr{Nothing},Int32) solve.p val
        end
    end
end
...
function set_presolve_type(solve::ClpSolve, value::Integer)
    ...
    @clpsolve_ccall setPresolveType Nothing (Ptr{Nothing},Int32,Int32) solve.p value -1
end
...
function set_solve_type(solve::ClpSolve, value::Integer)
    ...
    @clpsolve_ccall setSolveType Nothing (Ptr{Nothing},Int32,Int32) solve.p value -1
end
...
function get_extra_info(solve::ClpSolve, which::Integer)
    ...
    @clpsolve_ccall getExtraInfo Int32 (Ptr{Nothing},Int32) solve.p which
end
